...
void qmk_settings_reset(void) {
    ...
    // Bit 1 is "auto shift modifiers": without it, holding a key while Ctrl or
    // Gui is down types the unshifted glyph, so Ctrl+hold(p) never reaches
    // Ctrl+Shift+P and Gui+hold(,) never reaches the emoji picker.
    QS.auto_shift = 1 | 2;
